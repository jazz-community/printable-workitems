define([
	...
	function (declare, require, DojoSweetAlert, PlanModeClient, PlanChooserWidget) {
		...
		return declare("com.siemens.bt.jazz.viewlet.printableworkitems.jazzUtilities.modules.src.WorkItemPlanDialog", [], {

			constructor: function (props) {

			},

			getListFromPlan: function () {
											...
											planModeTableRow.onclick = function (event) {
												...
												future.setCallback(function (result) {
													...
													onPlanModes.setCallback(function (success) {
														...
														xhr.open(
															"POST", "".concat(
																url && url.length > 0 ? url : 'https://localhost:7443/jazz', "/service/com.ibm.team.workitem.common.internal.rest.IWorkItemRestService/getWorkItems")
														);
													...
													PlanningClient2.getInstance().fetchPlanItems(
														fakePlanDataHolder.getItemId(),
														planModeList.fPlanModes[planID].fPlanMode.fSource.itemId,
														Flags.NONE,
														onPlanModes
													);
												...
												PlanningClient2.getInstance().getItemStore().withItem(
													UIItemHandle.fromHandle(result1),
													future,
													com.ibm.team.apt.api.client.util.Flag.Refresh
												);
