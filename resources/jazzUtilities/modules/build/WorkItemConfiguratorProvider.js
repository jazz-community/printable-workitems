define(['dojo/on', 'dojo/dom-style', 'dojo/dom-class', 'dojo/mouse', 'dojo/_base/declare', 'dojo/_base/lang', 'require', './PrintableWorkitemPanelDialog', './PrintableWorkItemDraw', './../DojoSweetAlert', "./ProcessAttachments", 'dojo/text!./../ConfiguratorUI.html'], function (on, domStyle, domClass, mouse, declare, lang, require, PrintableWorkitemPanelDialog, PrintableWorkItemDraw, DojoSweetAlert, ProcessAttachments, template) {
  ...
  return declare("com.siemens.bt.jazz.viewlet.printableworkitems.jazzUtilities.modules.build.WorkItemConfiguratorProvider", dojo.global.com.ibm.team.dashboard.web.ui.CustomPreferenceProvider, {
     ...
     * @override super
     */
    createChooser: function createChooser(onOk, settings, viewlet) {
     ...
     */
    configLocationChangedRedraw: function configLocationChangedRedraw() {
      var _this = this;
      ...
      if (this.checkExternalConfiguration) {
        ...
        new ProcessAttachments().getWebLoadAllProcessAttachments(function (successful, attachmentList, message) {
          ...
          if (attachmentList.length > 0) {
            ...
            _this._setCurrentSelectedAttachment(_this.useExternalConfiguration == "" ? attachmentList[0] : _this.useExternalConfiguration);
     ...
     */
    redrawPanelContent: function redrawPanelContent() {
      ...
      this._printableWorkItemDraw.drawTableFromConfiguration(this.useWorkitemUUID, [fakeConfiguration], // Update Title function gets called
      true, // Don't update Dynamic Values, if already loaded
      true, // Should detailed children be loaded
      this.useDetailedChildren, // Should the print be optimized
      null, // Are there predefined attributes
      null, // Should dynamic values not be shown in the UI
      true);

      this.getTemplateDOMElementByID("data\\.height").value = this._getCurrentConfiguration().config.height;
      this.getTemplateDOMElementByID("data\\.width").value = this._getCurrentConfiguration().config.width; // Sets the config border, if not defined

      if (this._getCurrentConfiguration().config.border === undefined) {
        this._getCurrentConfiguration().config.border = true;
      }

      this.getTemplateDOMElementByID("data\\.border").checked = this._getCurrentConfiguration().config.border; // Sets the position of the table to default, if not defined

      if (this._getCurrentConfiguration().config.tablePosition === undefined) {
        this._getCurrentConfiguration().config.tablePosition = "center";
      }
     ...
     */
    _recalculateRegionIDs: function _recalculateRegionIDs() {
     ...
     */
    _getNextRegionID: function _getNextRegionID() {
     ...
     * @default #ffffff
     */
    _addNewRegion: function _addNewRegion(start, end) {
      ...
      var backColor = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : "#ff0000";
      ...
      var textColor = arguments.length > 9 && arguments[9] !== undefined ? arguments[9] : "#ffffff";
     ...
     */
    _removeRegion: function _removeRegion(regionID) {
      for (var i = 0; i < this._getCurrentConfiguration().values.length; i++) {
        var element = this._getCurrentConfiguration().values[i];
     ...
     * @default null
     */
    _addNewType: function _addNewType(typeName) {
     ...
     */
    _deleteType: function _deleteType(typeName) {
      var _this2 = this;
     ...
     */
    _switchType: function _switchType(typeName) {
      for (var i = 0; i < this.currentConfiguration.length; i++) {
        var element = this.currentConfiguration[i];
     ...
     */
    _renameType: function _renameType(newName) {
      for (var i = 0; i < this.currentConfiguration.length; i++) {
        var element = this.currentConfiguration[i];
     ...
     */
    getHolderElement: function getHolderElement() {
     ...
     */
    getTemplateElementFullIDByID: function getTemplateElementFullIDByID(value) {
     ...
     */
    getTemplateDOMElementByID: function getTemplateDOMElementByID(value) {
     ...
     */
    getAllTemplateDOMElementsByID: function getAllTemplateDOMElementsByID(value) {
     ...
     */
    verifyCurrentEditorStatus: function verifyCurrentEditorStatus(verify) {
     ...
     */
    _getRegionConfiguration: function _getRegionConfiguration(regionID) {
      if (regionID === null) {
        return null;
      }
     ...
     */
    calculateIDByPosition: function calculateIDByPosition(x, y) {
     ...
     */
    calculatePositionByID: function calculatePositionByID(tableID) {
      if (tableID === null) {
        return {
          x: "-",
          y: "-"
     ...
     */
    _generateConfiguratorUI: function _generateConfiguratorUI() {
      this.getDOMElement().appendChild(this.configuratorUITemplate(template));

      this._bindEventsToUIElements(); //this.redrawPanelContent();
     ...
     */
    _bindEventsToTable: function _bindEventsToTable() {
        ...
        on(element, "click", function (evt) {
          if (self.verifyCurrentEditorStatus(self.currentEditorStatusEnum.region_create)) {
            if (self._firstClickedID === null) {
              ...
              self._addNewRegion(self.calculatePositionByID(self._firstClickedID), self.calculatePositionByID(element.getAttribute('tableid')));
            }
     ...
     */
    _bindEventsToUIElements: function _bindEventsToUIElements() {
      var _this3 = this;
      ...
      on(this.getTemplateDOMElementByID("button\\.fullscreen"), "click", function (evt) {
        ...
        if (self._getFullScreenStatus()) {
          ...
          var dialogBackground = dialog.querySelector(":scope > .background");
      ...
      on(document, "fullscreenchange, mozfullscreenchange, webkitfullscreenchange, msfullscreenchange", function (evt) {
        // Dialog
        var d = self.getDOMElement().closest('[role="dialog"]'); // SWAL
      ...
      on(this.getTemplateDOMElementByID("data\\.width"), "change", function (evt) {
        ...
        ;
        _this3._getCurrentConfiguration().config.width = newValue; // Update the width of all the Regions that have dynamic width enabled
     ...
     */
    _overwriteConfigurationRequest: function _overwriteConfigurationRequest(element) {
      var _this4 = this;
     ...
     */
    _updateDataValues: function _updateDataValues() {
      this.getTemplateDOMElementByID("data\\.fields").innerHTML = this._getCurrentConfiguration().config.height * this._getCurrentConfiguration().config.width;
      this.getTemplateDOMElementByID("data\\.region\\.fields").innerHTML = this.currentSelectedRegion === null ? "-" : this.getHolderElement().querySelectorAll(":scope > .workitemTable > tr > td[regionid=\"".concat(this.currentSelectedRegion, "\"]")).length;
      this.getTemplateDOMElementByID("data\\.regions").innerHTML = this._getCurrentConfiguration().values.length;
    },
     ...
     * @default null
     */
    _updateTypes: function _updateTypes() {
     ...
     */
    _updateSelectedRegion: function _updateSelectedRegion(regionID) {
     ...
     */
    _updateCurrentPositionInTable: function _updateCurrentPositionInTable(tableID) {
     ...
     * @default true
     */
    _updateRegionViewValues: function _updateRegionViewValues() {
     ...
     */
    _saveNewValuesToConfiguration: function _saveNewValuesToConfiguration() {
      if (this.currentSelectedRegion !== null) {
        var regionConfig = this._getRegionConfiguration(this.currentSelectedRegion);
     ...
     */
    _markBorderOfRegion: function _markBorderOfRegion(regionToShow) {
     ...
     */
    _invertColor: function _invertColor(hex) {
     ...
     */
    _padZero: function _padZero(str, len) {
      len = len || 2;
     ...
     */
    _getFullScreenStatus: function _getFullScreenStatus() {
     ...
     */
    _openFullScreen: function _openFullScreen(elem) {
     ...
     */
    _closeFullScreen: function _closeFullScreen() {
     ...
     */
    _setCurrentSelectedAttachment: function _setCurrentSelectedAttachment(attachmentName) {
      var _this5 = this;
      ...
      new ProcessAttachments().getWebContentProcessAttachment(attachmentName, function (successful, resultJSON, message) {
        if (successful) {
          ...
          _this5._updateSelectedRegion(null); //this.redrawPanelContent();
     ...
     */
    _sweetAlertSelectTextAll: function _sweetAlertSelectTextAll() {
      setTimeout(function () {
     ...
     */
    _uploadConfigurationFormText: function _uploadConfigurationFormText() {
      var _this6 = this;
     ...
     */
    _uploadConfigurationFromFile: function _uploadConfigurationFromFile() {
      var _this7 = this;
     ...
     */
    _uploadAndParseConfiguration: function _uploadAndParseConfiguration(newConfiguration) {
      var _this8 = this;
