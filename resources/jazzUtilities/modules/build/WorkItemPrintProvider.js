define(['dojo/on', 'dojo/_base/declare', 'dojo/_base/lang', 'require', './PrintableWorkitemPanelDialog', './PrintableWorkItemDraw', './../DojoSweetAlert', './WorkItemPlanDialog', 'dojo/text!./../PrintUI.html', 'dojo/text!./../../../templates/Workitem.css', "com.ibm.team.dashboard.web.ui.DashboardConstants", 'com.ibm.team.workitem.viewlets.web.ui.internal.utils.WorkItemQueryChooser'], function (on, declare, lang, require, PrintableWorkitemPanelDialog, PrintableWorkItemDraw, DojoSweetAlert, WorkItemPlanDialog, template, printStyle) {
  ...
  return declare("com.siemens.bt.jazz.viewlet.printableworkitems.jazzUtilities.modules.build.WorkItemPrintProvider", dojo.global.com.ibm.team.dashboard.web.ui.CustomPreferenceProvider, {
    ...
    createChooser: function createChooser(onOK, settings, viewlet) {
      ...
      this.getDOMElement().appendChild(this.printUITemplate(template));
      ...
      on(this.getTemplateDOMElementByID("select\\.pageOptimize"), "change", function (evt) {
        try {
          ...
          swal("Invalid Print-Config", "The selected Print-Config isn't valid: \n\"".concat(evt.target.selectedOptions[0].label, "\""), "error"); // Change back to the Default/Null value, with no Print-Config
        ...
        if (self._globalLastQueryDto === null && self._planWorkItemList.length == 0) {
          ...
          self._handleFoundItemList(self._planWorkItemList);
        } else {
      ...
      on(this.getTemplateDOMElementByID("button\\.print"), "click", function (evt) {
        try {
          ...
          printWindow.document.getElementsByTagName("body")[0].appendChild(self.getTemplateDOMElementByID("holder\\.table").cloneNode(true));
          ...
          printWindow.close();
          swal("Printing Error", "An error has occurred, while trying to print the configuration. Please try something from the list below.\n- Try again later\n- Reload the page\n- Change your browser (It's recommended to use Chrome)", "error");
        }
      });
      ...
      on(this.getTemplateDOMElementByID("button\\.plan\\.select"), "click", function () {
        var mainContainer = _this;
        new WorkItemPlanDialog().getListFromPlan().then(function (data) {
          mainContainer._printableWorkitemPanelDialog._dialog._primaryHeaderText.innerText = "".concat(mainContainer._globalDialogPrimaryTitle, ", Plan is selected");

          mainContainer._handleFoundItemList(data);
        }, function (error) {
          console.warn(error);
        });
      });
      ...
      on(this.getTemplateDOMElementByID("button\\.query\\.select\\.allAdvanced"), "click", function () {
        swal({
          ...
          var inputList = [];
          inputQuery.replace(/\;$/g, '').replace(/start/ig, 0).replace(/end/ig, Number.MAX_SAFE_INTEGER).replace(/ /g, '').split(";").forEach(function (element) {
     ...
     * @default null
     */
    _createSinglePrintableConfiguration: function _createSinglePrintableConfiguration() {
      ...
      this._printableWorkItemDraw.drawTableFromConfiguration(_id == null ? this._globalSingleWorkItemID : _id, this._globalConfiguration, // Update Title function gets called
      true, // Update Dynamic Values, if already loaded
      false, // Should detailed children be loaded
      this.useDetailedChildren, // Use a predefined size for better printing
      this._globalPrintConfig, // Custom Attribute
      _customAttribute);
     ...
     */
    _applyFoundQuery: function _applyFoundQuery(queryDto) {
      this._globalLastQueryDto = queryDto;
      this._printableWorkitemPanelDialog._dialog._primaryHeaderText.innerText = "".concat(this._globalDialogPrimaryTitle, ", Query: ").concat(queryDto.name);
      window.scrollTo(0, 0);
      swal({
      ...
      xhr.addEventListener("readystatechange", function () {
        ...
        if (this.readyState === 4) {
          if (this.status.toString().startsWith("2")) {
            var resultList = JSON.parse(this.responseText)['soapenv:Body'].response.returnValue.value.rows; //Clear the Table
     ...
     * @default false
     */
    _handleFoundItemList: function _handleFoundItemList(itemList) {
      ...
      this._createSinglePrintableConfiguration(this._queryList[this._queryListID], false, this._getAttributesForWorkItem(this._queryList[this._queryListID]));
    },
     ...
     */
    _getAttributesForWorkItem: function _getAttributesForWorkItem(workitemID) {
      var returnValue = [];

      if (this._planWorkItemList.length != 0) {
        var planData = this._planWorkItemList.find(function (element) {
          return element.id == workitemID;
        });
        ...
        var queryElement = this._queryWorkItemDataList.find(function (element) {
          return element.id == workitemID;
        });
     ...
     */
    _querySelectorUpdate: function _querySelectorUpdate() {
      var _this3 = this;

      this.getTemplateDOMElementByID("query\\.select").querySelectorAll("[type=checkbox]").forEach(function (element) {
        _this3.getDOMElement().querySelector(":scope .workitemTable[queryid='".concat(element.getAttribute("workitemid"), "']")).style.display = element.checked ? "inline-table" : "none";
      });
     ...
     */
    _queryResultSelectAll: function _queryResultSelectAll() {
        ...
        if (!element.disabled) {
          ...
          if (elementID && localItemID) {
            switch (mode) {
              ...
              case 3:
                var planListData = self._planWorkItemList.find(function (x) {
                  ...
                  if (localItemID >= dataElement.start && localItemID <= dataElement.end) {
                    if (dataElement.extraValue && dataElement.extraValue.length != 0) {
                      ...
                      for (var extraValueIndex = 0; extraValueIndex < dataElement.extraValue.length; extraValueIndex++) {
                        ...
                        if (!extraValueAllowed && data.length == 1 || extraValueAllowed && data.length > 1) {
                          break;
                        }
                      }
